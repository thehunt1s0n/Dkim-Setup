...
with open("/etc/opendkim/key.table", "w") as f:
    key_table_line = f"{base_domain}    {domain}:20230815:/etc/opendkim/keys/{base_domain}.private"
    f.write(key_table_line)

#Signing.table config
with open("/etc/opendkim/signing.table", "w") as f:
    signing_table_line = f"*@{domain}    {base_domain}"
...
print(f"OpenDKIM has been installed and configured for the domain {domain}.")
